...
默认优先走 OpenAI Images API。
只有显式要求 chat endpoint 或传入 reference_image_path 时，才走
chat completions 流式返回图片 URL / base64 的路径。
"""
...
class GptImage2Generator:
    ...
    def _request_via_images(self, prompt: str, size: str) -> str:
        ...
        print(f"[request] POST {url} size={size} quality={self.quality}")
    ...
    def generate_scene_image(
        ...
        expected_ratio = parse_size_ratio(target_size) or ASPECT_RATIO_VALUES.get(self.aspect_ratio)
        print(f"[prompt] prompt[:100]: {prompt[:100].replace(chr(10), ' ')}{'...' if len(prompt) > 100 else ''}")
        ...
        for ratio_attempt in range(MAX_ASPECT_RETRIES + 1):
            ...
            if aspect_acceptable(width, height, expected_ratio):
                print(f"[OK] 已保存: {output_path} ({width}x{height}, 请求尺寸 {target_size})")
                return output_path

            actual_ratio = width / height if height else 0
            expected = expected_ratio or ASPECT_RATIO_VALUES.get(self.aspect_ratio, 1.0)
...
if __name__ == "__main__":
    generator = GptImage2Generator(aspect_ratio="1:1")
    generator.generate_scene_image(
        {"index": 0, "image_prompt": "A clean futuristic poster with the title gpt-image-2 test"},
        "test_output.png",
    )
