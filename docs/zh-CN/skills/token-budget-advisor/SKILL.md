...
description: >-
  在回答前，为用户提供关于要消耗多少回复深度的知情选择。
  当用户明确想控制回复长度、深度或 token 预算时，应使用此技能。
  ...
  corta", "quiero controlar cuánto usas"，或其他明确要求控制回答长度或深度的变体。
  DO NOT TRIGGER when: 用户在当前会话中已经指定了一个级别（保持该级别）、请求显然只需要一个词回答，或 "token" 指的是认证、会话或支付 token，而不是回复长度。
origin: community
---
