...
description: "将 Claude Code 改造成具备持久记忆、定时运行、计算机操作和任务排队能力的全自主代理系统。通过利用 Claude Code 原生的 cron、调度、MCP 工具和记忆机制，替代独立代理框架（Hermes、AutoGPT）。适用于用户需要持续自主运行、定时任务或自驱动代理循环时。"
origin: ECC
---
